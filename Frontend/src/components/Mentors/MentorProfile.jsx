...
const MentorProfile = ({ token }) => {
  ...
  return (
    ...
    {showForm && (
      <div className="mb-6">
        <MentorProfileEdit mentor={mentor} setMentor={setMentor} setShowForm={setShowForm} />
      </div>
    )}
      ...
      <div className="text-center">
          <h1 className="text-gray-900 text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold text-center underline" style={{ textShadow: '2px 2px 4px rgba(0,0,0,0.15)' }}>
            {mentor.firstName + " " + mentor.lastName}
          </h1>
        </div>
        <div className="flex items-center justify-center">
          {/* Project Category */}
  <p className="font-bold text-xl text-gray-700 mr-2">Project Category:</p>
  <span className="text-blue-500 text-xl font-bold">{mentor.projectCategory}</span>
</div>
{/* Profile Photo */}
        <div className="flex justify-center items-center px-4 pt-6">
        <img
            src={mentor.profilePhoto ? mentor.profilePhoto : 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?q=80&w=1964&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'}
            alt= "Mentor"
...
<p className="font-bold text-xl text-gray-700 mb-1">Question:</p>
          <span className="text-gray-900 text-xl italic">{mentor.questionToAsk}</span>
        </div>
    {/* Mentor's email */}

    <div className="bg-sky-100 p-4 rounded-md text-center">
  <p className="italic text-gray-500 text-sm mb-1">Only visible to you</p>
  <div className="flex items-center justify-center">
    <p className="font-bold text-xl text-gray-700 mr-2">Email:</p>
    <span className="text-blue-500 text-xl font-bold">{mentor.email}</span>
  </div>
</div>
